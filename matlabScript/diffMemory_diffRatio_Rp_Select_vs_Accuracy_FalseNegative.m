function diffMemory_diffRatio_Rp_Select_vs_Accuracy_FalseNegative()
    ...
    figure
    hold on;    
    %sh+replace+select+ratio1
    errorbar(3:7, accuracy(2,2,1,3:end), accuracySD(2,2,1,3:end), '-rv', 'linewidth', 2);
    %sh+replace+select+ratio2
    errorbar(3:7, accuracy(2,2,2,3:end), accuracySD(2,2,2,3:end), '-g>', 'linewidth', 2);
    errorbar(3:7, accuracy(2,2,3,3:end), accuracySD(2,2,2,3:end), '-b+', 'linewidth', 2);
    errorbar(3:7, accuracy(2,2,4,3:end), accuracySD(2,2,2,3:end), '-mo', 'linewidth', 2);

    set(gca, 'FontSize', 18,'XTick', 1:7, 'xticklabel', {2060, 10302, 20604, 51510, 103020, 154530, 206040});
    legend('sample rate*1','sample rate*2','sample rate*4','sample rate*8');
    ...
    hold on;    
    %sh+replace+select+ratio1
    errorbar(3:7, fn(2,2,1,3:end), fnSD(2,2,1,3:end), '-rv', 'linewidth', 2);
    %sh+replace+select+ratio2
    errorbar(3:7, fn(2,2,2,3:end), fnSD(2,2,2,3:end), '-g>', 'linewidth', 2);
    errorbar(3:7, fn(2,2,3,3:end), fnSD(2,2,3,3:end), '-b+', 'linewidth', 2);
    errorbar(3:7, fn(2,2,4,3:end), fnSD(2,2,4,3:end), '-mo', 'linewidth', 2);

    set(gca, 'FontSize', 18,'YTick', 0.01:0.02:0.15,'XTick', 1:7, 'xticklabel', {2060, 10302, 20604, 51510, 103020, 154530, 206040});
    legend('sample rate*1','sample rate*2','sample rate*4','sample rate*8');    legend('Location', 'northeast');
    legend('boxoff');
    xlabel('#buckets in hashmap')
